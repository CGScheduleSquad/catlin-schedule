...
window.addEventListener('load', () => {
  ...
  let events = ical[2];
    ...
    return {date:dtstart, startTime:new Date(0, 0, 0, dtstart.getHours(), dtstart.getMinutes()), title:summary.join(" - "), subtitle:"Block "+description[0].Block, rowSpan: 1, numBlocks: events.filter(a => getDT('start', a[1]).getDate()===dtstart.getDate()).length}
  });

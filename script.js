...
window.addEventListener('load', () => {
  try {
    ...
    console.log(ical);
    let events = ical[2];
      allEvents = events.filter(a => a[1].length === 8).map(a => {
...
function prepRegularSchedule(daySchedule) {
    ...
    for (let i = 0; i < newBlocks.length; i++) { // Replace empty blocks with free blocks and extend long blocks into the next period
        ...
        if (i < newBlocks.length - 1 && (newBlocks[i].subtitle.match(/\dL/) != null || newBlocks[i].title === 'Free' || newBlocks[i].title === 'Assembly') && newBlocks[i + 1] == null) {
            newBlocks[i].rowSpan++;
